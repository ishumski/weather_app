import SVGIconTemplate from './svg-icon-template'

interface FahrenheitIconProps {
  width: string
  height: string
  viewBox: string
  fill: string
}

const FahrenheitIcon = ({
  width,
  height,
  viewBox,
  fill
}: FahrenheitIconProps) => {
  return (
    <SVGIconTemplate
      width={width}
      height={height}
      viewBox={viewBox}
      fill={fill}
      d="M13.033 11.079c-0.225-0.212-0.487-0.38-0.784-0.504-0.301-0.121-0.621-0.184-0.961-0.184-0.341 0-0.662 0.063-0.961 0.184-0.297 0.124-0.559 0.292-0.784 0.504-0.224 0.213-0.4 0.463-0.529 0.753-0.128 0.288-0.191 0.592-0.191 0.912 0 0.32 0.064 0.625 0.191 0.913 0.129 0.288 0.305 0.539 0.529 0.751 0.225 0.213 0.487 0.383 0.784 0.504 0.299 0.124 0.62 0.185 0.961 0.185 0.34 0 0.66-0.061 0.961-0.185 0.297-0.121 0.559-0.29 0.784-0.504 0.224-0.212 0.4-0.464 0.526-0.751 0.128-0.288 0.194-0.592 0.194-0.913 0-0.319-0.066-0.624-0.194-0.912-0.126-0.289-0.302-0.539-0.526-0.753zM12.074 13.496c-0.215 0.214-0.477 0.32-0.785 0.32s-0.57-0.107-0.785-0.32c-0.212-0.213-0.32-0.463-0.32-0.753 0-0.297 0.108-0.552 0.32-0.759 0.215-0.208 0.477-0.311 0.785-0.311s0.57 0.103 0.785 0.311c0.211 0.207 0.319 0.462 0.319 0.759 0 0.289-0.108 0.539-0.319 0.753zM23.176 12.552v-1.969h-7.599v11.025h2.191v-4.496h4.624v-1.967h-4.624v-2.592h5.407z"
    />
  )
}

export default FahrenheitIcon
